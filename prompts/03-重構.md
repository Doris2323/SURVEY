# 04 - 重構階段

## 目標

在測試保護下，改善程式碼品質。

## ⚠️ 前置條件

**只處理沒有 `@ignore` 標籤的 Feature**

- 帶有 `@ignore` 的 Feature 尚未進入開發流程，會被 `npm test` 跳過
- 只有已完成綠燈階段（測試通過）的 Feature 才能進入重構階段
- 如果目標 Feature 還有 `@ignore`，請先回到 [01-紅燈.md](./01-紅燈.md) 完成前置步驟

## ⚠️ 最重要的規則：絕對禁止修改 .feature 檔案

**在整個 BDD 流程中，絕對不能修改 `.feature` 檔案。**

- Feature 檔案是需求規格（Specification），是需求契約
- 重構階段只改善程式碼品質，不改變行為
- **絕對不能**因為重構而修改 Feature 檔案
- **絕對不能**因為想要「更好的測試」而修改 Feature 檔案

**你的職責**：
- ✅ 重構業務邏輯（`src/*.js`）
- ✅ 重構測試程式碼（Step Definition）
- ✅ 保持測試通過
- ❌ **絕對不能**修改 Feature 檔案
- ❌ **絕對不能**因為重構而改變 Feature 的語意

## TDD 重構原則

在 TDD 中，**重構**是在不改變程式行為的前提下，改善程式碼結構。

重構的前提：
1. 所有測試必須通過（綠燈）
2. 每次小改動後都要執行測試
3. 測試失敗就回退
4. **絕對不修改 Feature 檔案**（需求規格應保持穩定）

## 重構時機

### ✅ 適合重構

- 重複的程式碼
- 過長的函式
- 魔術數字（Magic Numbers）
- 命名不清楚

### ❌ 不適合重構

- 測試還沒通過
- 趕時間交付
- 沒有測試保護

## 常見重構技巧

### 1. 抽取常數

```javascript
// Before
function addToCart(productId) {
  const sheet = getSheet('購物車');
  // ...
}

// After
const SHEET_NAME = '購物車';

function addToCart(productId) {
  const sheet = getSheet(SHEET_NAME);
  // ...
}
```

### 2. 抽取輔助函式

```javascript
// Before
function addToCart(productId) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('購物車');
  if (!sheet) {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    sheet = ss.insertSheet('購物車');
    sheet.appendRow(['商品ID', '數量', '單價', '建立時間']);
  }
  // ...
}

// After
function getSheet(name) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sheet = ss.getSheetByName(name);
  if (!sheet) {
    sheet = ss.insertSheet(name);
    sheet.appendRow(['商品ID', '數量', '單價', '建立時間']);
  }
  return sheet;
}

function addToCart(productId) {
  const sheet = getSheet(SHEET_NAME);
  // ...
}
```

### 3. 改善命名

```javascript
// Before
function a(p, q) {
  const s = getSheet(SHEET_NAME);
  const d = new Date();
  s.appendRow([p, q, d]);
}

// After
function addToCart(productId, quantity) {
  const sheet = getSheet(SHEET_NAME);
  const now = new Date();
  const cartItem = createCartItem(productId, quantity, now);
  sheet.appendRow(cartItemToRow(cartItem));
}
```

### 4. 結構化資料

```javascript
// Before
sheet.appendRow([productId, quantity, price, time]);

// After
function createCartItem(productId, quantity, price) {
  return {
    productId: productId,
    quantity: quantity,
    price: price,
    createdAt: formatDateZhTW(new Date())
  };
}

function cartItemToRow(item) {
  return [item.productId, item.quantity, item.price, item.createdAt];
}
```

## 重構流程

```
┌─────────────────────────────────────────────────┐
│                                                 │
│   1. 確認測試全部通過（綠燈）                   │
│        │                                        │
│        ▼                                        │
│   2. 做一個小改動                               │
│        │                                        │
│        ▼                                        │
│   3. 執行測試                                   │
│        │                                        │
│        ▼                                        │
│   4. 測試通過？                                 │
│        │                                        │
│        ├── 是 ──→ 回到步驟 2（繼續重構）        │
│        │                                        │
│        └── 否 ──→ 回退改動，回到步驟 2          │
│                                                 │
└─────────────────────────────────────────────────┘
```

## 🎯 常見重構模式

### 1. 抽取欄位索引常數

```javascript
// ✅ 推薦：使用命名常數
const COLUMN = {
  PRODUCT_ID: 0,
  QUANTITY: 1,
  PRICE: 2,
  CREATED_AT: 3
};

function getCartItems() {
  // ...
  const productId = row[COLUMN.PRODUCT_ID];    // 清楚
  const quantity = row[COLUMN.QUANTITY];       // 清楚
}

// ❌ 避免：魔術數字
function getCartItems() {
  const productId = row[0];   // 0 是什麼？
  const quantity = row[1];    // 1 是什麼？
}
```

### 2. 抽取狀態常數

```javascript
// ✅ 推薦
const ORDER_STATUS = {
  PENDING: 'PENDING',
  PAID: 'PAID',
  SHIPPED: 'SHIPPED',
  COMPLETED: 'COMPLETED'
};

if (status === ORDER_STATUS.PAID) {
  // ...
}

// ❌ 避免：字串字面值
if (status === 'PAID') {  // 容易打錯
  // ...
}
```

### 3. 抽取日期判斷邏輯

```javascript
// ✅ 推薦：輔助函式
function getTodayDateRange() {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const tomorrow = new Date(today);
  tomorrow.setDate(tomorrow.getDate() + 1);
  return { today, tomorrow };
}

function isToday(createdAtISO) {
  const { today, tomorrow } = getTodayDateRange();
  const createdAt = new Date(createdAtISO);
  return createdAt >= today && createdAt < tomorrow;
}

function getTodayOrders() {
  // ...
  if (isToday(row[COLUMN.CREATED_AT])) {
    orders.push(...);
  }
}

// ❌ 避免：重複的日期邏輯
function getTodayOrders() {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  // ... 在每個函式都重複
}
```

### 4. 抽取驗證邏輯

```javascript
// ✅ 推薦
function validateCartItem(productId, quantity) {
  if (!productId) {
    return { valid: false, message: '請選擇商品' };
  }
  if (quantity <= 0) {
    return { valid: false, message: '數量必須大於 0' };
  }
  return { valid: true };
}

function addToCart(productId, quantity) {
  const validation = validateCartItem(productId, quantity);
  if (!validation.valid) {
    return { success: false, message: validation.message };
  }
  // ...
}

// ❌ 避免：驗證邏輯散落各處
function addToCart(productId, quantity) {
  if (!productId) {
    return { success: false, message: '請選擇商品' };
  }
  if (quantity <= 0) {
    return { success: false, message: '數量必須大於 0' };
  }
  // ... 很長的函式
}
```

### 5. 抽取工作表操作

```javascript
// ✅ 推薦
function getSheet(name) {
  return SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName(name);
}

function getAllRows(sheetName) {
  return getSheet(sheetName).getDataRange().getValues();
}

// ❌ 避免：重複的工作表操作
function addToCart(productId) {
  const sheet = SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName('購物車');
  // ...
}

function getCartItems() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet()
    .getSheetByName('購物車');
  // ... 重複的程式碼
}
```

## 重構檢查清單

- [ ] 抽取常數（避免魔術數字/字串）
- [ ] 抽取輔助函式（避免重複程式碼）
- [ ] 改善命名（清楚表達意圖）
- [ ] 結構化資料（使用物件而非陣列）
- [ ] 每次改動後測試都通過
- [ ] **日期判斷邏輯已抽取**
- [ ] **欄位索引使用常數而非數字**
- [ ] **狀態值使用常數而非字串**

## 完成條件

```bash
npm test
```

```
15 scenarios (15 passed)
77 steps (77 passed)
```

程式碼符合以下標準：
- [ ] 沒有重複的程式碼
- [ ] 函式長度適中（<20 行）
- [ ] 命名清楚
- [ ] 常數已抽取
- [ ] 所有測試通過

## 下一步

重構完成後，可以：
1. 提交程式碼（`git commit`）
2. 推送到 Apps Script（`clasp push`）
3. 部署 Web App 測試
4. 開始下一個 TDD 循環
