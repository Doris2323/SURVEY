...
1. **不要加 Before hook** - `this.ctx` 已由 `features/support/hooks.js` 初始化
...
如果 feature 使用新的工作表，需在 `features/support/hooks.js` 註冊：
...
將完整的 step definitions 存到 `features/step_definitions/[feature-name].steps.js`
